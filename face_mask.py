...
change_face_list = glob.glob(f"{path_faces}*.jpg")
...
def draw_triangulated_mesh(ocv_keypoints, img):
    for i in range(0, int(len(mp_triangulation) / 3)):
        points = [mp_triangulation[i * 3], mp_triangulation[i * 3 + 1], mp_triangulation[i * 3 + 2]
                  ]
...
def get_mask(image_file_name, face_mesh = None):
...
def process_base_face_mesh(drawing_spec, face_mesh, image_file, show_landmarks=False, show_triangulated_mesh=False):
    ...
    base_face_handler = get_mask(image_file, face_mesh)
...
def get_bounding_box(base_input_image, landmark_base_ocv, show_triangulated_mesh=False):

    # copy
    base_input_image = base_input_image.copy()

    # show triangulated mesh
    if show_triangulated_mesh:
        base_input_image = draw_triangulated_mesh(landmark_base_ocv, base_input_image)

    # find bounding box
    x_min = base_input_image.shape[1]  # start at far right
    x_max = 0
    y_min = base_input_image.shape[0]  # start at top
    y_max = 0
    for landmark_coords in landmark_base_ocv:
        x = landmark_coords[0]
        y = landmark_coords[1]
        x_min = min(x_min, x)
        x_max = max(x_max, x)
    ...
    # crop
    # img_debug_input_mask = base_input_image[y_min:y_max, x_min:x_max]
    x_center = (x_max + x_min)/2
    y_len = y_max - y_min  # 480
    x_len = y_len * 640 / 480  # keep aspect ratio 640 x 480
    img_debug_input_mask = base_input_image[y_min:y_max, math.floor(x_center-x_len/2):math.ceil(x_center+x_len/2)]

    return img_debug_input_mask

def main():

    # initialize
    triangle_indexes = mp_triangulation
    ...
    base_face_handler, landmark_base_ocv, base_input_image = process_base_face_mesh(
        ...
        show_triangulated_mesh=True  # key_draw_landmarks
    )

    # process mask image for debug window
    img_debug_input_mask = get_bounding_box(base_input_image, landmark_base_ocv, True)

    # capture real stream, process, and forward to virtual cam
    ...
    with pyvirtualcam.Camera(width=640, height=480, fps=60) as cam:
        ...
        while cap.isOpened():
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    if len(landmark_target_ocv) > 0:
                        ...
                        if process == True:
                            for i in range(0, int(len(mp_triangulation) / 3)):
                                triangle_index = [mp_triangulation[i * 3],
                                                mp_triangulation[i * 3 + 1],
                                                mp_triangulation[i * 3 + 2]]
                            ...
                            # get all white face behind mask (no mouth), black background
                            img2_new_face_white = np.clip(img2_new_face, 0, 1) * 255
                            ...
                            img2_head_real = cv2.bitwise_and(seam_clone, seam_clone, mask=img2_head_mask)
                            ...
                            #                                   img2_head_mask, center_face2, cv2.MIXED_CLONE)  # mouth not cut out
            ...
            # draw debug window
            # ie 2x2 view of important stages
            cv2.imshow(
                "debug",
                cv2.resize(
                    np.concatenate(
                        (
                            np.concatenate(
                                (
                                    # cv2.resize(base_face_handler["img"], dsize=(seamlessclone.shape[1], seamlessclone.shape[0])),
                                    cv2.resize(img_debug_input_mask, dsize=(seamlessclone.shape[1], seamlessclone.shape[0])),
                                    out_image
                                ),
                                axis=1
                            ),
            ...
            # send to virtual cam
            cam.send(cv2.cvtColor(seamlessclone, cv2.COLOR_BGR2RGB))
            
            # keyboard actions

            key = cv2.waitKey(5)  # returns ASCII int
